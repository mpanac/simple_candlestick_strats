...
def process_param_combination(df, params, freq, fees, init_cash):
    param_dict = dict(zip(['atr_multiplier', 'ewm_period', 'envelopes_perc'], params))
...
if __name__ == "__main__":
    ...
    strategy_module = importlib.import_module('strats.candlestick_reversion_envelopes_upgraded')
    strategy_name = strategy_module.__name__.split('.')[-1]

    input_file = './data/binance_data/btcusdt_ohlcv_1h.csv'  # Change the data path to use different granularity. DON'T FORGET TO ALSO CHANGE FREQ variable !!!
    ...
    n, window_len, set_lens = wfo_rolling_split_params(total_candles=len(raw_ohlcv), insample_percentage=0.80, n=5)
    ...
    param_ranges = {
        'atr_multiplier':range(2, 8),
        'ewm_period':  range(5, 51, 5),
        'envelopes_perc': [round(x, 3) for x in np.arange(0.004, 0.0261, 0.002)]
        
    }
    
    
    freq = '1h' # Don't forget to change the freq based on the provided data granularity (eg.: 15m, 30m, 1h, 4h ..)
